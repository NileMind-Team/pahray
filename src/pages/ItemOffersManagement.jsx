  ...
  if (Array.isArray(errorData.errors)) {
    const errorMessages = errorData.errors.map((error) => {
      switch (error.code) {
        ...
        case "ItemOffer.InvalidStartDate":
          return "تاريخ البداية غير صالح.";
        case "ItemOffer.InvalidEndDate":
          return "تاريخ النهاية غير صالح.";
        default:
          // إذا كان هناك وصف، حاول ترجمته
          if (error.description) {
            const descLower = error.description.toLowerCase();

            // ترجمة الأوصاف الإنجليزية الشائعة
            if (descLower.includes("start date must be in the future")) {
              return "تاريخ البداية يجب أن يكون في المستقبل.";
            }
            if (descLower.includes("end date must be after start date")) {
              return "تاريخ النهاية يجب أن يكون بعد تاريخ البداية.";
            }
            if (descLower.includes("discount value must be greater than 0")) {
              return "قيمة الخصم يجب أن تكون أكبر من الصفر.";
            }
            if (descLower.includes("menu item not found")) {
              return "العنصر المحدد غير موجود.";
            }
            if (descLower.includes("branch not found")) {
              return "الفرع المحدد غير موجود.";
            }
            if (descLower.includes("offer already exists")) {
              return "هناك عرض نشط لهذا العنصر بالفعل.";
            }
      ...
      if (
        ...
        if (Array.isArray(errorValue)) {
          errorValue.forEach((msg) => {
            ...
            if (msgLower.includes("required")) {
              errorMessages.push(`حقل ${key} مطلوب`);
            } else if (msgLower.includes("invalid")) {
              errorMessages.push(`قيمة ${key} غير صالحة`);
            } else {
              errorMessages.push(msg);
            }
          });
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <motion.button
                ...
                className={`flex items-center gap-2 bg-gradient-to-r from-[#4945E7] to-[#6A67F0] text-white px-4 sm:px-5 py-3 rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300 text-sm sm:text-base whitespace-nowrap w-full sm:w-auto justify-center`}
              >
              ...
              >
                <div className="bg-white/90 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-xl sm:shadow-2xl p-4 sm:p-5 lg:p-6 border border-gray-200/50 dark:bg-gray-800/90 dark:border-gray-600 sticky top-4 sm:top-6 transition-colors duration-300">
                  ...
                  >
                                ...
                                <div>
                                  {menuItems.map((item) => (
                                    <div
                                      ...
                                      className="px-4 py-3 hover:bg-gradient-to-r hover:from-[#f0f2ff] hover:to-[#e0e5ff] dark:hover:from-gray-600 dark:hover:to-gray-500 cursor-pointer text-gray-700 dark:text-gray-300 transition-all text-sm sm:text-base border-b border-gray-100 dark:border-gray-600 last:border-b-0 flex flex-col text-right"
                                    >
                    ...
                    <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-[#f0f2ff] to-[#e0e5ff] dark:from-gray-700 dark:to-gray-600 rounded-xl border border-[#6A67F0]/30 dark:border-gray-500">
                      <input
                        type="checkbox"
                        name="isEnabled"
